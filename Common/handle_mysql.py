...
class HandleMysql:
    ...
    def __init__(self, config=do_yaml.read_yaml("mysql", "config")):
        # yaml中获取连接数据库的相关信息
        self.config = config
        # 连接数据库
...
if __name__ == '__main__':
    ...
    print(HandleMysql().read_mysql("select new_order_contents from t_cloud_printer_config where sn=%s", "testWap4")[
              "new_order_contents"])
