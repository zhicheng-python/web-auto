...
class DeviceMoreInfoAssert(BPO):
    ...
    def click_device_log_assert(self):
        ...
        try:
            self.wait_element_visible(loc.device_log_jump_to, module="点击设备LOG-进入设备log页面，跳转元素可见")
        except:
            return False
        else:
            return True

    def click_log_button_to_on_assert(self):
        logger.info("断言：点击设备LOG，进入设备页面，点击log开关，开启log元素存在，数据库数据比对，返回True or False")


        try:
            self.wait_element_visible(loc.device_log_button_on, module="点击设备LOG，进入设备页面，点击log开关，开启log元素存在")
        except:
            value = 0
        else:
            value = 1

        db_value = HandleMysql().read_mysql(do_yaml.read_yaml("mysql","query_log_switch_sql"),DMIPD.SN)[DMSPD.log_switch]
        logger.info(f"----db_value----:{db_value}")

        if value ==1 and db_value==1:
            return True
        else:
            return False
    ...
    def click_log_button_to_off_assert(self):
        ...
        try:
            self.wait_element_visible(loc.device_log_button_off, module="点击log开关，关闭log元素存在")
        except:
            value = 0
        else:
            value = 1

        db_value = HandleMysql().read_mysql(do_yaml.read_yaml("mysql", "query_log_switch_sql"), DMIPD.SN)[
            DMSPD.log_switch]
        logger.info(f"----db_value----:{db_value}")
